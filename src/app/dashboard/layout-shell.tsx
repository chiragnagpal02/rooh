...
const NAV_ITEMS: {
  ...
  },
  {
    ...
    description: "Key practical details extracted from recordings - insurance, bank info, policies, and contacts.",
    icon: (
      <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
    ...
    <>
      ...
      {open && typeof window !== 'undefined' && (
        <div style={{
          ...
          <p style={{ fontSize: '12px', color: '#E8E0D5', margin: 0, lineHeight: 1.6 }}>
            {description}
          </p>
          <div style={{
            position: 'absolute',
            ...
            background: '#2C2926',
            borderLeft: '0.5px solid #3C3936',
            borderBottom: '0.5px solid #3C3936',
          }} />
        </div>
      )}
    </>
...
function MobileInfoSheet({ item, onClose }: { item: typeof NAV_ITEMS[0]; onClose: () => void }) {
  return (
    <div
      onClick={onClose}
      style={{
    ...
    >
      <div
        onClick={e => e.stopPropagation()}
        style={{
          background: '#FDF8F3', borderRadius: '16px 16px 0 0',
          padding: '24px', width: '100%',
          border: '0.5px solid #E8E0D5',
        }}
      >
        <div style={{ width: '36px', height: '4px', background: '#E8E0D5', borderRadius: '2px', margin: '0 auto 20px' }} />
        <p style={{ fontSize: '16px', fontWeight: 500, color: '#1C1917', margin: '0 0 8px' }}>{item.label}</p>
        <p style={{ fontSize: '14px', color: '#78716C', margin: '0 0 20px', lineHeight: 1.6 }}>{item.description}</p>
        <button
          onClick={onClose}
          style={{
            width: '100%', padding: '12px', background: '#1C1917',
            color: '#FDF8F3', border: 'none', borderRadius: '10px',
...
export default function LayoutShell({
  ...
  const [mobileInfoItem, setMobileInfoItem] = useState<typeof NAV_ITEMS[0] | null>(null)
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", background: "#FDF8F3" }}>
      ...
      {mobileInfoItem && (
        <MobileInfoSheet item={mobileInfoItem} onClose={() => setMobileInfoItem(null)} />
      )}

      {/* Sidebar */}
      <div style={{ width: sidebarWidth, background: "#1C1917", display: "flex", flexDirection: "column", flexShrink: 0, position: "sticky", top: 0, height: "100vh", transition: "width 0.2s ease", overflow: "visible", zIndex: 30 }}>
        ...
        <div style={{ flex: 1, padding: "12px 8px", display: "flex", flexDirection: "column", gap: "2px" }}>
          {NAV_ITEMS.map((item) => (
            <div key={item.key} style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
              ...
              {/* Info button — desktop tooltip, mobile sheet */}
              {!collapsed && (
                <>
                  {/* Desktop */}
                  ...
                  <button
                    ...
                    onClick={e => { e.stopPropagation(); setMobileInfoItem(item) }}
                    style={{
                      width: '16px', height: '16px', borderRadius: '50%',
                      border: '1px solid #3C3936', background: 'transparent',
                      color: '#5F5E5A', fontSize: '10px', fontWeight: 500,
        ...
        <div style={{ padding: "12px 8px", borderTop: "0.5px solid #2C2926" }}>
          <button onClick={handleSignOut} title={collapsed ? "Sign out" : undefined} style={{ display: "flex", alignItems: "center", gap: "10px", padding: collapsed ? "9px 0" : "9px 10px", justifyContent: collapsed ? "center" : "flex-start", borderRadius: "8px", cursor: "pointer", background: "none", border: "none", color: "#5F5E5A", fontSize: "13px", width: "100%" }}>
            <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
        <div style={{ background: "#FDF8F3", borderBottom: "0.5px solid #E8E0D5", padding: "14px 24px", display: "flex", flexDirection: "column", gap: "8px", position: "sticky", top: 0, zIndex: 10 }} className="topbar">
          <div>
        ...
        <div style={{ flex: 1, padding: "32px 24px", maxWidth: "720px", width: "100%", margin: "0 auto", boxSizing: "border-box" }}>
      ...
      <style>{`
        @media (hover: hover) {
          .desktop-info { display: flex; align-items: center; }
