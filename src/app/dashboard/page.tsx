...
export default function Dashboard() {
  ...
  async function fetchData() {
    ...
    if (family) setParentName(family.parent_name);
  ...
  const hasPracticalData = Object.values(entities).some((arr) => arr.length > 0);
    ...
    <main style={{ background: "#FDF8F3", minHeight: "100vh" }}>
      ...
      <nav style={{
        ...
        <a href="/" style={{ textDecoration: "none" }}>
          <svg width="80" height="26" viewBox="0 0 120 38" xmlns="http://www.w3.org/2000/svg">
                ...
                {[
                  {
                    ...
                    color: "#3C3489",
                    bg: "#EEEDFE",
                    render: (v: any) => [v.name, v.role, v.contact].filter(Boolean).join(" · "),
                  },
                  {
                    ...
                    color: "#085041",
                    bg: "#E1F5EE",
                    render: (v: any) => [v.name, v.frequency].filter(Boolean).join(" · "),
                  },
                  {
                    ...
                    color: "#633806",
                    bg: "#FAEEDA",
                    render: (v: any) => [v.provider, v.number, v.type].filter(Boolean).join(" · "),
                  },
                  {
                    key: "bank" as const,
                    label: "Bank accounts",
                    color: "#712B13",
                    bg: "#FAECE7",
                    render: (v: any) => [v.name, v.branch, v.details].filter(Boolean).join(" · "),
                  },
                  {
                    ...
                    color: "#0C447C",
                    bg: "#E6F1FB",
                    render: (v: any) => [v.name, v.relation, v.number].filter(Boolean).join(" · "),
                  },
                  {
                    ...
                    color: "#444441",
                    bg: "#F1EFE8",
                    render: (v: any) => [v.description, v.location].filter(Boolean).join(" · "),
                  },
            ...
            {filtered.map((recording) => {
              ...
              return (
                <div
                  ...
                  style={{
                    background: "white",
                    border: isExpanded ? "1px solid #D6CEC4" : "0.5px solid #E8E0D5",
                    borderRadius: "16px",
                    overflow: "hidden",
                      ...
                      {recording.primary_type === "legacy" && (
                        <div style={{
                          ...
                          <p style={{ fontSize: "13px", color: "#633806", margin: 0 }}>
                            This is a personal message kept in {parentName}'s exact words. Listen to the original recording above.
                          </p>
                        </div>
                      )}
                            ...
                            {Object.entries(recording.extracted_entities).map(([key, values]) =>
                              ...
                              values.map((v: any, i: number) => (
                                <span key={`${key}-${i}`} style={{
                                  ...
                                  <span style={{ color: "#1D9E75", marginRight: "4px", fontWeight: 500 }}>
                                    {key}
                                  </span>
